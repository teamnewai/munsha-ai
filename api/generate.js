  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: 'You are an expert in Saudi organizational design. Respond ONLY with valid JSON. No markdown, no backticks. All values in Arabic except enum values.',
        messages: [{ role: 'user', content: prompt }]
      })
    });
    const data = await response.json();
    if (!response.ok) return res.status(500).json({ error: data?.error?.message || 'خطأ' });
