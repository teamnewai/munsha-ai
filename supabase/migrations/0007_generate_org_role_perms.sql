-- generate_org (نهائي): + صلاحيات افتراضية لكل منصب حسب الأقدمية.
-- القادة (مدير/رئيس/مشرف/قائد): صلاحيات كاملة؛ الموظفون: عرض/إضافة/تعديل؛
-- + صلاحية المالية لإدارة المالية. يبني على 0006.
create or replace function public.generate_org(
  p_input jsonb, p_structure jsonb, p_source text default 'local', p_employees jsonb default '[]'::jsonb
)
...
declare
  ...
  dep jsonb; sec jsonb; rol jsonb; emp jsonb; v_title text; v_perms jsonb;
...
begin
  ...
  select org_id, role into v_org, v_role from memberships where user_id = uid order by (role='owner') desc limit 1;
  ...
  values (v_org, coalesce(p_source,'local'),
          ...
          v_depts, 'STR-'||to_char(now(),'YYYYMMDD')||'-'||upper(substr(replace(v_org::text,'-',''),1,4)))
  ...
  for dep in select * from jsonb_array_elements(v_depts) loop
    ...
    for rol in select * from jsonb_array_elements(coalesce(dep->'roles','[]'::jsonb)) loop
      v_title := rol#>>'{}';
      is_lead := v_title ~ 'مدير|رئيس|مشرف|قائد';
      v_perms := case when is_lead
        then '["view","create","edit","delete","approve","reports","manage_members"]'::jsonb
        else '["view","create","edit"]'::jsonb end;
      if dep->>'key' = 'finance' then v_perms := v_perms || '["finance"]'::jsonb; end if;
      if not exists (select 1 from org_roles where org_id=v_org and dept_key=dep->>'key' and title=v_title) then
        insert into org_roles (org_id, dept_key, title, perms) values (v_org, dep->>'key', v_title, v_perms);
      end if;
    end loop;
  ...
  return json_build_object('ok', true, 'org_id', v_org, 'doc_id', v_doc,
    'departments', jsonb_array_length(v_depts), 'employees', v_emp_count, 'ai_agents', v_agents);
end;
$function$;
